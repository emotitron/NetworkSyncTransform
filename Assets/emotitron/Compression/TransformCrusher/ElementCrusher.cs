	...
	public class ElementCrusher : Crusher<ElementCrusher>, IEquatable<ElementCrusher>, ICrusherCopy<ElementCrusher>
	{
		...
		public void ApplyWorldCrusherSettings()
		{
			...
			if (useWorldBounds)
			{
				...
				if (WorldBoundsSO.single == null)
				{
					WorldBoundsSO.OnSingletonReady -= OnWorldBoundsReady;
					WorldBoundsSO.OnSingletonReady += OnWorldBoundsReady;
					return;
				}
		...
		private WorldBoundsGroup GetUsedWorldBounds()
		{
			...
			{
				// If this is no longer a existing worldbounds layer, reset to default
				if (boundsGroupId >= WorldBoundsSO.Single.worldBoundsGroups.Count)
					boundsGroupId = 0;

				var wbs = WorldBoundsSO.Single.worldBoundsGroups[boundsGroupId];
				return wbs;
			}
			else return null;
		}
		...
		public void Write(CompressedElement ce, ref Bitstream bitstream, BitCullingLevel bcl = BitCullingLevel.NoCulling)
		{
			if (TRSType == TRSType.Quaternion)
			{
				_qcrusher.Write((ulong)ce.cQuat, ref bitstream);
			}

			else if (TRSType == TRSType.Scale && cache_uEnabled)
			{
				_ucrusher.Write(ce.cUniform, ref bitstream, bcl);
			}

			else
			{
				if (cache_xEnabled)
					_xcrusher.Write(ce.cx, ref bitstream, bcl);
				if (cache_yEnabled)
					_ycrusher.Write(ce.cy, ref bitstream, bcl);
				if (cache_zEnabled)
					_zcrusher.Write(ce.cz, ref bitstream, bcl);
			}
		}
		...
		public Element Read(ref Bitstream buffer, BitCullingLevel bcl = BitCullingLevel.NoCulling)
		{
			...
			if (TRSType == TRSType.Quaternion)
			{
				return _qcrusher.Decompress(buffer.Read(cache_qBits));
			}

			else if (cache_isUniformScale)
			{
				float val = _ucrusher.ReadAndDecompress(ref buffer, bcl);
				return new Vector3(val, val, val);
			}

			Debug.Log("Read Element " + cache_xEnabled);
			return new Vector3(
				cache_xEnabled ? _xcrusher.ReadAndDecompress(ref buffer, bcl) : 0f,
				cache_yEnabled ? _ycrusher.ReadAndDecompress(ref buffer, bcl) : 0f,
				cache_zEnabled ? _zcrusher.ReadAndDecompress(ref buffer, bcl) : 0f
				);
		}
		...
		public void Read(CompressedElement nonalloc, ref Bitstream bitstream, BitCullingLevel bcl = BitCullingLevel.NoCulling)
		{
			...
			{
				CompressedFloat cx = cache_xEnabled ? new CompressedFloat(_xcrusher, (uint)bitstream.Read(cache_xBits[(int)bcl])) : new CompressedFloat();
				CompressedFloat cy = cache_yEnabled ? new CompressedFloat(_ycrusher, (uint)bitstream.Read(cache_yBits[(int)bcl])) : new CompressedFloat();
				CompressedFloat cz = cache_zEnabled ? new CompressedFloat(_zcrusher, (uint)bitstream.Read(cache_zBits[(int)bcl])) : new CompressedFloat();
		...
		public void CompressAndWrite(Vector3 v, ref Bitstream bitstream, IncludedAxes ia = IncludedAxes.XYZ)
		{
			...
			else if (_trsType == TRSType.Quaternion)
			{
				...
				if (cache_qEnabled)
					bitstream.Write(_qcrusher.Compress(Quaternion.Euler(v)), cache_qBits);
			}
			else
			{
				//FloatCrusherUtilities.CheckBitCount(xcrusher.GetBits(0) + ycrusher.GetBits(0) + zcrusher.GetBits(0), 96);
				...
				if (cache_xEnabled)
					bitstream.Write(_xcrusher.Compress(v.x), cache_xBits[0]);
				if (cache_yEnabled)
					bitstream.Write(_ycrusher.Compress(v.y), cache_yBits[0]);
				if (cache_zEnabled)
					bitstream.Write(_zcrusher.Compress(v.z), cache_zBits[0]);
		...
		public int TallyBits(BitCullingLevel bcl = BitCullingLevel.NoCulling)
		{
			...
			else
			{
				if (_trsType == TRSType.Position && useWorldBounds)
				{
					return WorldBoundsSO.TallyBits(ref boundsGroupId);
				}
				return
	...
	public class ElementCrusherDrawer : CrusherDrawer
	{
		...
		public override void OnGUI(Rect r, SerializedProperty property, GUIContent label)
		{
			...
			if (target.TRSType == TRSType.Position && !isWorldBounds)
			{
				...
				EditorGUI.LabelField(new Rect(fcLeft2 + 14, currentline, enumwidth - 10, LINEHEIGHT), wb_gc, (GUIStyle)"MiniLabel");
				bool useWrldBnds = GUI.Toggle(new Rect(fcLeft2, currentline, 16, LINEHEIGHT), useWorldBounds.boolValue, GUIContent.none, (GUIStyle)"OL Toggle");
			...
			if (target.enableLocalSelector && (target.TRSType != TRSType.Position || !target.UseWorldBounds))
			{
				...
				GUI.Label(new Rect(paddedwidth - localtoggleleft + 14, currentline, 80, LINEHEIGHT), GC_LCL, (GUIStyle)"MiniLabel");
