...
namespace emotitron.Compression
{
	...
	public class WorldBoundsSO : SettingsScriptableObject<WorldBoundsSO>
	{
		...
		//{
		//	var single = Single;
		//	if (single != null && single.worldBoundsGroups.Count > 0)
		...
		const float helpboxhght = WorldBoundsGroup.BoundsReportHeight * 11 + 6; //42f  ;
	...
	public class WorldBoundsSOEditor : Editor
	{
		public override void OnInspectorGUI()
		{
			...
			WorldBoundsSO.Single.DrawGui(target, false, false, true);

			//EditorGUI.BeginDisabledGroup(true);

			//var wbgs = serializedObject.FindProperty("worldBoundsGroups");
			//for (int i = 0; i < wbgs.arraySize; ++i)
			//{
			//	EditorGUILayout.LabelField(wbgs.GetArrayElementAtIndex(i).FindPropertyRelative("name").stringValue);
			//	EditorGUILayout.PropertyField(wbgs.GetArrayElementAtIndex(i).FindPropertyRelative("crusher"));
			//}
